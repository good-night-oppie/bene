...
*Indexed by `idx_agents_status_v2` (`status, created_at`), `idx_agents_created_at` (`created_at`), and `idx_agents_parent` (`parent_id`) — see the [index catalog](#index-catalog).*
...
|---|---|---|---|---|
| agents | `idx_agents_status_v2` | `status, created_at` | No | Filter agents by lifecycle state |
| agents | `idx_agents_created_at` | `created_at` | No | List recent agents |
