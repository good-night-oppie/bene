...
*Indexed by `idx_agents_status` (`status`), `idx_agents_parent` (`parent_id`), and `idx_agents_created_at` (`created_at`) — see the [index catalog](#index-catalog).*
...
| agents | `idx_agents_created_at` | `created_at` | No | Order agents by creation time |
