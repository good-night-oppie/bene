...
## Worked example: the `submit_side_effect` primitive

This cycle produced `submit_side_effect`, validating four runtime-boundary primitives along the way: `start_or_signal`, `business_idempotency_key`, `side_effect_label`, and (eventually) `submit_side_effect`.

**Reviewing in isolation surfaced one idiom.** Reading the first three primitives on their own suggested encoding the time bucket directly into the workflow ID with `int(timestamp / delta_burst)`, which makes TTL semantics tumbling-window by construction and bounds workflow history naturally. It became the recommended idiom for time-bounded entity actors.

**Writing real code against them surfaced the real gap.** The decisive step was generating an actual consumer — a burst aggregator, with tests — *as if the primitives already existed*. That exposed what the abstract sketch could not: there was no atomic verb for the check-then-write pattern that external writes (a Jira POST, say) require. Splitting it into `check_side_effect` then `record_side_effect` leaves a window where an activity retry after a crashed worker re-executes the external write — duplicate Jira comments in production. The fix is a single atomic verb, `submit_side_effect(label, business_key, execute_fn) -> result`, where the runtime owns the lock around the whole operation. That is the core lesson of the cycle: a gap invisible in a design sketch becomes obvious the moment you write code against it.

**An honest contract beat an over-promise.** The runtime cannot actually guarantee atomicity across an external system like Jira — only across its own ledger and lease. So the honest contract is: the runtime guarantees atomic ledger transitions (`pending → executing → done | failed`) and lease exclusivity, but for external writes the `SideEffectLabel` must declare a `reconciliation_strategy` (`IDEMPOTENT_BY_KEY`, `READBACK_SENTINEL`, `READBACK_RESULT_KEY`, or `MANUAL`) that retry uses to detect whether the prior attempt actually committed. A related constraint: the shared DTOs cannot live under `bene/temporal/`, because a local runtime must implement the same protocol — they belong in a runtime-neutral module like `bene/runtime/core/`.
