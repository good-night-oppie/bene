...
| **锁定真凶 (Failure localization)** | 就因为跑圈引擎默认会给每个工具留下印记，所以你拿着 `bene failure localize <agent>` 去抓虫时，它能在**真实的运行现场**里，一眼揪出那个最早让系统开始崩盘的致命失误 —— 这玩意不用你开启任何开关，也不用你去手动伪造现场。（去查阅 [CLI 指令大黄页 (cli-reference.md)](cli-reference.md#bene-failure-localize)。） |
...
这批是在内核层面对运行时 (runtime) 做出的修复，已经随 `0.2.1` 这个发布版一起打进包里了：
