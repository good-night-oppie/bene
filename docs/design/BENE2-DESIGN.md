...
- Kernel tables are created lazily on first kernel command or `attach_kernel` (sane defaults, no config required); a `bene.yaml` `kernel:` section now wires runner ContextOS and loop-guard middleware, while autonomy defaults and consolidation schedule wiring remain planned.
