...
CREATE INDEX IF NOT EXISTS idx_agents_created_at_v2 ON agents(created_at DESC);
...
# Migration to v7: fast compound index for fetching recent agents
# Eliminates SQLite Temp B-Tree on `ORDER BY created_at DESC` when loading dashboard and UI.
MIGRATION_V7_SQL = """
CREATE INDEX IF NOT EXISTS idx_agents_created_at_v2 ON agents(created_at DESC);
"""
