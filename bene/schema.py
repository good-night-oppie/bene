...
-- ⚡ Bolt Optimization: Fast index for recent agent fetching to avoid O(N log N) sorting
...
# Migration to v7: index on agents(created_at DESC)
# Eliminates SQLite Temp B-Tree on `ORDER BY created_at DESC` when loading dashboard
MIGRATION_V7_SQL = """
CREATE INDEX IF NOT EXISTS idx_agents_created_at_v7 ON agents(created_at DESC);
"""
