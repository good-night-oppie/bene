...
# Migration to v6: fast compound indexes for shared_log
# Eliminates SQLite Temp B-Tree on `ORDER BY position` for read and tally queries.
MIGRATION_V6_SQL = """
...
# Migration to v7: index on agent creation time
# Eliminates SQLite Temp B-Tree when sorting agents by created_at DESC (e.g. dashboard queries).
MIGRATION_V7_SQL = """
-- ⚡ Bolt Optimization: Index on created_at to avoid Temp B-Trees on window scans
-- Impact: Avoids O(N log N) sort step per agent query, ensuring O(1) fetch times for recent agents.
CREATE INDEX IF NOT EXISTS idx_agents_created_at ON agents(created_at);
"""
