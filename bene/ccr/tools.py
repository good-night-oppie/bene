...
class ToolRegistry:
    ...
    def __init__(self, afs: Bene, permission_policy: ToolPermissionPolicy | None = None):
        self.afs = afs
    ...
    async def execute(self, agent_id: str, tool_name: str, arguments: dict[str, Any]) -> Any:
    ...
    def _register_builtins(self) -> None:
        ...
        self.register(
            ToolDefinition(
                name="squirrel_localpath",
                description=(
                    "Translate a Jira / gravytrain log URL to a local NFS-mounted "
                    "filesystem path using the upstream `squirrel localpath` tool. "
                    "Prefer this over fetching URLs: the local path supports grep, "
                    "python, and other tooling directly. Returns the local path on "
                    "success or a 'not found / not available' marker."
                ),
                parameters={
                    "type": "object",
                    "properties": {
                        "url": {
                            "type": "string",
                            "description": "Remote URL (e.g. an https://...oppie.com/build/... link)",
                        },
                    },
                    "required": ["url"],
                },
                handler=self._squirrel_localpath,
                timeout_seconds=15,
            )
        )
    ...
    def _squirrel_localpath(self, url: str, **kwargs) -> str:
        """Wrap `squirrel localpath <url>`; degrade gracefully when missing."""
        try:
            result = subprocess.run(
                ["squirrel", "localpath", url],
                capture_output=True,
                text=True,
                timeout=10,
            )
        except FileNotFoundError:
            return "squirrel binary not available on PATH; URL→fs translation skipped"
        except subprocess.TimeoutExpired:
            return "squirrel localpath timed out; URL→fs translation skipped"

        if result.returncode != 0:
            return f"squirrel localpath: not found (stderr: {result.stderr.strip()[:200]})"
        path = result.stdout.strip()
        if not path:
            return "squirrel localpath: not found (empty result)"
        return path
