...
Implements Algorithm 1 from the Meta-Harness paper (arXiv:2603.28052):
  ...
  3. For N iterations:
     ...
     c. Evaluate valid harnesses (parallel, each gets a BENE agent)
...
from bene.metaharness.pareto import compute_pareto, ParetoFrontier
...
class MetaHarnessSearch:
    ...
    Uses BENE agents for isolation:
        ...
        for iteration in range(1, self.config.max_iterations + 1):
            ...
            try:
                ...
                logger.error(
                    ...
                    iteration,
                    self.config.proposer_timeout_seconds,
                )
    ...
    async def resume(self, search_agent_id: str) -> SearchResult:
        ...
        config_data = json.loads(self.afs.read(search_agent_id, "/config.json").decode())
        ...
        for iteration in range(last_iteration + 1, self.config.max_iterations + 1):
            iter_start = time.time()
            logger.info(
                "=== Iteration %d / %d (resumed) ===", iteration, self.config.max_iterations
            )
    ...
    def _update_stagnation(self, frontier: ParetoFrontier, iteration: int) -> int:
        ...
        epsilon = 0.001
        improved = any(
            abs(curr_best.get(obj, 0.0) - prev_best.get(obj, 0.0)) > epsilon for obj in curr_best
        ) or (
            len(frontier.points)
            > (self.afs.get_state_or(self.search_agent_id, "prev_frontier_size") or 0)
        )
        ...
        should_pivot = stagnant >= threshold and (
            pivot_fired_at is None or stagnant - pivot_fired_at >= threshold
        )
        if should_pivot:
    ...
    def _load_skills_from_knowledge(self, agent_id: str) -> None:
        ...
        try:
            knowledge_id = self.afs.get_or_create_singleton(
                "bene-knowledge",
    ...
    def _store_result(
        ...
        candidate_engram_id = self._bridge_candidate_to_evolve(harness, result, iteration)
        # CORAL Tier 2: compact summary in /attempts/ for fast proposer scanning
        prev_best: dict = self.afs.get_state_or(self.search_agent_id, "prev_best_scores") or {}
        if self.config.auto_promote and result.scores:
            self._maybe_auto_promote(harness, candidate_engram_id, result.scores, prev_best)
        epsilon = 0.001
        if not prev_best:
            attempt_status = "neutral"
        elif any(
            result.scores.get(obj, 0.0) - prev_best.get(obj, 0.0) > epsilon for obj in result.scores
        ):
            attempt_status = "improved"
        elif any(
            prev_best.get(obj, 0.0) - result.scores.get(obj, 0.0) > epsilon for obj in result.scores
        ):
    ...
    def _bridge_candidate_to_evolve(
        self,
        harness: HarnessCandidate,
        result: EvaluationResult,
        iteration: int,
    ) -> str:
        """Persist an evaluated meta-harness candidate as a tier-4 genome engram.

        This deliberately stops short of promotion: the candidate is now visible
        to the existing probe/promote path, where promote() still requires an
        ACCEPT verdict linked to this engram.
        """
        ...
        candidate_data = harness.to_dict()
        candidate_data["scores"] = result.scores
        genome = genome_from_candidate(candidate_data)
        genome.scores = dict(result.scores)

        metadata = {
            "source": "bene.metaharness",
        ...
        engram_id = store.append(
            "strategic",
            f"mh-candidate:{harness.harness_id}",
            genome.encode(),
            tier=4,
            provenance={"agent_id": self.search_agent_id},
        ...
        self.afs.write(
            self.search_agent_id,
            f"/harnesses/{harness.harness_id}/evolve.json",
            json.dumps(
                {
                    "engram_id": engram_id,
                    "genome_id": genome.genome_id,
                    "scores": genome.scores,
                    "promotion_requires": metadata["promotion_requires"],
                },
                indent=2,
            ).encode(),
        )
    ...
    def _maybe_auto_promote(
        ...
        """Opt-in (config.auto_promote): gate-promote the bridged candidate if it
        beats prev-best on the primary objective. Still behind the kill gate —
        an improvement probe must ACCEPT before promote() fires."""
        from bene.kernel import EngramStore, ensure_v2
        from bene.kernel.evolve import auto_promote_evolved

        metric = self._primary_objective(scores)
        if metric is None:
            return
        ensure_v2(self.afs.conn)
        store = EngramStore(self.afs.conn, self.afs.blobs)
        outcome = auto_promote_evolved(
            candidate_engram_id,
            metric=metric,
            subject={k: float(v) for k, v in scores.items()},
            baseline={k: float(v) for k, v in (baseline or {}).items()},
            store=store,
            conn=self.afs.conn,
            delta=self.config.auto_promote_delta,
            probe_name=f"mh-autopromote:{harness.harness_id}",
        )
        self.afs.write(
            self.search_agent_id,
            f"/harnesses/{harness.harness_id}/promotion.json",
            json.dumps(outcome.to_dict(), indent=2).encode(),
        )

    def _primary_objective(self, scores: dict) -> str | None:
        """The objective to gate on: the first configured objective present in
        ``scores`` (sign prefix stripped), else the first score key."""
        for obj in self.config.objectives or []:
            name = obj.lstrip("+-")
            if name in scores:
                return name
    ...
    def _query_memory_for_context(self, query: str, limit: int = 5) -> str:
        ...
        try:
            ...
            if not hits:
                return ""
            lines = ["## Prior Results (from shared memory)\n"]
            for h in hits:
        ...
        history_text += json.dumps(
            {
                ...
                "points": [
                    {"harness_id": p.harness_id[:12], "scores": p.scores} for p in frontier.points
                ],
            }
        )
        self.afs.write(
