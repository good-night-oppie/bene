...
**Learning:** When querying SQLite with an `ORDER BY col1 DESC, col2 DESC` clause (like in `memory` cross-agent queries fetching recent records), if an index only covers `col1` or covers both but in default `ASC` order, SQLite falls back to a Temp B-Tree. This makes fetch times proportional to row count instead of a fast constant-time index lookup, which is a huge bottleneck as the table grows.
...
## 2025-03-02 - Avoid SQLite Temp B-Trees on ORDER BY Queries in agents

**Learning:** Dashboards heavily query the `agents` table with pagination limits using an `ORDER BY created_at DESC` clause. When no suitable index exists, SQLite builds a temporary B-Tree for every query — a cost that grows linearly with row count. (Note: SQLite B-trees are bidirectional, so a plain ASC index can serve DESC queries via reverse scan. The explicit `DESC` here is a defensive clarity choice that makes the intent self-documenting and guarantees the query plan — cost is identical.)
**Action:** Defined an index matching the `ORDER BY` direction used by the dashboards: `CREATE INDEX IF NOT EXISTS idx_agents_created_at ON agents(created_at DESC);`. Verify with `EXPLAIN QUERY PLAN` that it scans the index rather than building a temp B-tree.
