"""Regression tests for the goalcraft hardening findings F1-F6.

Every test is designed to FAIL against origin/main's scorer and PASS against
the hardened one. F6's ``SEED_PARTIAL`` import lives inside its test so that
F1-F5 stay individually diagnostic against the pre-fix module instead of all
dying on one collection-time ImportError.
"""
